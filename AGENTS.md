...
- Phone install + fast updates: see `docs/PHONE.md` (Expo Go for live reload; `npm run phone:build` = EAS APK, `npm run phone:update -- "msg"` = OTA update on the `preview` channel). JS-only changes ship via OTA; native changes need a new build. `npm run friends:update -- "msg"` publishes the same JS to the `friends` channel for iPhone users running it inside Expo Go (`app.config.js` switches the runtime version to `sdkVersion` via `LIFEOS_EXPO_GO`; never change `runtimeVersion` in `app.json` for that).
...
- Focus mode: `src/domain/focus.ts` (planned deep-work blocks; points only in `finishFocus`, ledger reason `focus_done`). The notification plan adds start/check-in/end pings and drops nudges + captures inside a block. Automatic prompts on app open (`useFocusPrompt` → `useChallengePrompt` → `useCapturePrompt`) must decide inside a short timeout and go through `canAutoPrompt()` in `src/notifications/taps.ts`, so they never stack on a screen a notification tap just opened. Tap routing lives ONLY in `openFromNotification()` there (de-duplicated per request id).
